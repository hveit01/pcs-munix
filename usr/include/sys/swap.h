...
typedef struct swaptab {
	...
	use_t	*st_ucnt;	/* Ptr to use count array for	*/
				/* pages on swap.		*/
	use_t	*st_last;
	long    st_swplo;       /* First block number on device */
				/* to be used for swapping.	*/
	long    st_npgs;        /* Number of pages of swap      */
				/* space on device.		*/
	long    st_nfpgs;       /* Nbr of free pages on device. */
} swpt_t;
...
/*      The following struct is used by the swapfunc system call.
 */

typedef struct swapint {
	short   si_cmd;         /* One of the command codes     */
				/* listed below.		*/
	char	*si_buf;	/* For an SI_LIST function, this*/
				...
				/* swap file.			*/
	long    si_swplo;       /* The first block number of the*/
				/* swap file.  Used only for	*/
				/* SI_ADD and SI_DEL.		*/
	long    si_nblks;       /* The size of the swap file in */
