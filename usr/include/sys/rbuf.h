...
typedef struct	rbuf
{
	...
	dev_t   b_dummy1;               /* b_dev */
	paddr_t b_dummy2;               /* b_paddr */
	unsigned b_dummy3;              /* b_bcount */
	union {
	    caddr_t b_addr;		/* low order core address */
	} b_un;

	daddr_t	b_blkno;		/* logical block # (computed from offset)*/
	char    b_dummy4;               /* b_error */
	struct  rbuf *f_forw;           /* position in RFS file hash table, b_resid */
	struct  rbuf *f_back;           /*  "  , b_start */
	long            b_fhandle;      /* buffer remote file ID , b_dummy */
	struct queue    *b_queue;       /* buffer remote server ID , b_proc */
	index_t         b_mntindx;      /* remote mount index , b_reltime */
	short   b_dummy5;               /* b_reltime second half */
