...
/* pcs the following loops with goto are weird, but the compiler produces 
 * the same code only with this construct. A while loop will reorder sequence */
loop0:      do {
                if (value >= sp->p10) {
                    value /= sp->p10;
                    *decpt += sp->n;
...
loop1:      if ((powtemp = 10.0 * pow10) <= value) {
                pow10 = powtemp;
                goto loop1;
            }
