/*PCS modified:
 * pcs has moved FP code into a special version in libm,
 * so this version is shorter and does not contain FP formatting */
#define NOFP
...
/*e8*/          { register long qval = val;
            ...
            if (flagword & DOTSEEN) {
/*e8*/              register leadzeroes = prec - (p - bp);
                if (leadzeroes > 0) {
