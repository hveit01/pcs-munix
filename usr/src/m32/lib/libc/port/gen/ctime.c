...
long    *tim;
{
    ...
    /* pcs new DST calculation for year > 1980 */
    } else if (ct->tm_year >= 80 && daylight != 0) {
        int maxday = 0, dstoff = 0, lastsun;
        switch (ct->tm_mon) {
        case 0:     /*jan*/
        case 1:     /*feb*/
        case 9:     /*oct*/
        ...
        case 2:     /*mar*/
            maxday += 25;
            lastsun = ct->tm_mday - ct->tm_wday;
            while (lastsun < maxday)
                lastsun += 7;
            if (ct->tm_mday > lastsun ||
                    (ct->tm_mday==lastsun && ct->tm_hour >= 1))
                dstoff = !dstoff;
            if (!dstoff)
                break;
            /*FALLTHRU*/
