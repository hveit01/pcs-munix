...
static char *_Version = "@(#) RELEASE:  1.1  Apr 21 1987 /usr/src/uv5.2/lib/libmalloc/malloc.c ";
...
/*     use level memory allocater (malloc, free, realloc)
    ...
    These blocks are of 3 types.
        ...
        3. A permanently allocated block.  This covers space
           aquired by the user directly through lsbrk().  It
...
/* PCS note that NDEBUG is not set, so the code has plenty of asserts.
 * Fortunately... */
...
char *malloc(nbytes)            /*PCS*/
long nbytes;                    /*PCS*/
{                               /*PCS*/
    extern char *lmalloc();     /*PCS*/
    return lmalloc(nbytes);     /*PCS*/
}                               /*PCS*/
        ...
        if (change == 0) {
            ...
            */
            ...
            holdhead = (struct holdblk**)
                lmalloc(sizeof(struct holdblk**) * (fastct + 1));
...
/*PCS*/ if (((char*)freeptr[1].nextblk - (char*)&(freeptr[1])) < nb) return 0;
        {
        ...
        if (blk == &(freeptr[1]))  {
            ...
            /* Three cases - 1. There is space between arenaend
                     ...
                     3. Case 1 is not true, and the last
                        block is free
            */
            if ((newblk = (struct header *)lsbrk(0)) != 
               (struct header *)((char *)arenaend + HEADSZ))  {
...
#endif
                /* get memory */
                if ((struct header *)lsbrk(nget) == 
                    (struct header *)-1) {
                        return 0;
                }
                /* add to arena */
                newend = (struct header *)((char *)newblk + nget - HEADSZ);
                ...
                if (lsbrk(nget) == (char *)-1) {
                    return 0;
/*450*/             }
                /* block must be word aligned */
/* line332 */   assert(((int)newblk%ALIGNSZ) == 0);
                newend =
                ...
                /* last block in arena is at end of memory and
                   is free */
/*PCS*/         nget = nb - ((char*)arenaend - (char*)lastblk);
                nget = (nget+BLOCKSZ-1)/BLOCKSZ*BLOCKSZ;
#ifdef pdp11
                if (newblk + nget + 64 < newblk) {
                    return NULL;
...
#endif
/* line 350*/   assert(((int)newblk%ALIGNSZ) == 0);
                if (lsbrk(nget) == (char *)-1)  {
                    return NULL;
                }
...
/*line420*/     assert((struct header *)lblk < arenaend);
/*line421*/     assert((struct header *)lblk > arena);
...
/*line452*/     ADDFREEQ(blk);
        /* see if we can compact */
        if (!TESTBUSY(nextnext = next->nextblk))  {
            do  {
/*line456*/     DELFREEQ(next);
                next = nextnext;
...
char *realloc(ptr, size)            /*PCS*/
register char *ptr;                 /*PCS*/
long size;                          /*PCS*/
{                                   /*PCS*/
    return lrealloc(ptr, size);     /*PCS*/
}                                   /*PCS*/
...
unsigned size;      /* size to change to */
{
    ...
    if (TESTSMAL(((struct lblk *)(ptr - MINHEAD))->header.holder))  {
        ...
        if(ptr != newptr)  {
            (void)lmemcpy(newptr,ptr,(int)size);
            free(ptr);
        }
    }  else  {
        blk = (struct header *)(ptr - minhead);
...
/*PCS*/     trusize = minhead; trusize = trusize + size;
...
/*line540*/     ADDFREEQ(newblk);
            ...
            newptr = lmalloc(size);
            if (newptr == NULL)  return NULL;
            (void)lmemcpy(newptr,ptr,(int)cpysize);
            free(ptr);
        }       
    }
    return newptr;
...
{
    register char *mp;
    register long nbytes = num * size; /*PCS*/

    mp = lmalloc(nbytes);
    if(mp == NULL)
        return(NULL);
    (void)lmemset(mp, 0, nbytes);
    return(mp);
}
...
/*PCS   if (freeptr->nextfree == GROUND)
           return (inf);*/
...
/*PCS*/     size = hblk->blksz + sizeof(struct lblk) - sizeof(int);
...
/*  space = ((char *)holdblk + HOLDSZ(size)) - (char *)unused;*/
    space += ((char *)holdblk - (char *)unused) + HOLDSZ(size);
