...
register FILE *iop;
{
	...
	register unsigned long nl;							/*pcs*/
	register int n;
	register unsigned char *cptr, *bufend;				/*pcs*/
	...
	if (iop->_base >= iop->_ptr)  {	/*this covers the unbuffered case, too*/
		if (((iop->_flag & _IONBF) != 0) || (nleft >= BUFSIZ))  {
			if ((nl=lwrite(fileno(iop),ptr,nleft)) != nleft) {	/*pcs*/
				iop->_flag |= _IOERR;
				nl = (nl >= 0) ? nl : 0;				/*pcs*/
			}
			return nl/size;								/*pcs*/
	...
	for (; ; ptr += n) {
        while ((n = bufend - (cptr = iop->_ptr)) <= 0)  /* full buf */
            if (_xflsbuf(iop) == EOF)
				return (count - (long)((nleft + size - 1)/size)); /*pcs*/
		n = MIN(nleft, n);
