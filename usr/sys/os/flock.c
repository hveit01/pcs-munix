/* @(#)flock.c  1.5 */
static char* _Version = "@(#) RELEASE:  1.0  Jul 09 1986 /usr/sys/os/flock.c ";
...
#define WAKEUP(ptr)     if (ptr->stat.wakeflg) { wakeup(ptr); ptr->stat.wakeflg = 0 ; }
#define l_end       l_len
...
struct flino *flip;
{
    if (--flip->fl_refcnt <= 0 && flip->fl_flck == 0) {
        flckinfo.filcnt--;
        if (flip->prev != 0)
