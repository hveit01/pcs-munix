/* @(#)sem.c    6.2 */
static char* _Version = "@(#) RELEASE:  1.3  Jul 29 1987 /usr/sys/os/sem.c ";
...
semctl()
{
    ...
    case SETVAL:
        ...
        if (u.u_exvec->exno == 0x2d) /* trap 13 */
            uap->arg >>= 16;
    ...
    case SETALL:
        ...
        for(i = 0;i < sp->sem_nsems;)
            if(semtmp.semvals[i++] > (ushort)seminfo.semvmx) {
                u.u_error = ERANGE;
                return;
            }
        semunrm(uap->semid, 0, sp->sem_nsems);
...
check:
    ...
    for(i = 0, op = semtmp.semops;i < uap->nsops;i++, op++) {
        ...
        if(op->sem_op > 0) {
            ...
            if(semp->semzcnt && semp->semval==0) {  /*pcs*/
                semp->semzcnt = 0;                  /*pcs*/
                wakeup(&semp->semzcnt);             /*pcs*/
            }                                       /*pcs*/
            continue;
        }
        if(op->sem_op < 0) {
