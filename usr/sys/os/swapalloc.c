/* @(#)swapalloc.c	1.2 */
static char *_Version = "@(#) RELEASE:  1.1  Jul 16 1986 /usr/sys/os/swapalloc.c ";
			...
			/* block to be removed.			*/
{
	...
	register swpt_t	*st;
	register retval;
	register int swpi = dbd->dbd_swpi;
	extern int comswapsmi;					/*pcs*/
	...
	if(retval == 0) {
		...
		if (comswapsmi == swpi && ownswap==0)	/*pcs*/
			diskless_swfree1(dbd);				/*pcs*/
	}
...
int		nblks;		/* Nbr of blocks to use.	*/
{
		...
		if(st->st_ucnt == NULL){
			...
			if((st->st_flags & ST_INDEL)  &&
			   (st->st_npgs == (nblks >> DPPSHFT))){
				...
				if ((sdev = swapdev) != dev)				/*pcs*/
					availsmem += st->st_npgs;	/*pcs*/
				return(smi);
			}
			u.u_error = EEXIST;
			return(-1);
	...
	u.u_error = 0;
	(*bdevsw[bmajor(dev)].d_open)(minor(dev), 3, 3);
	if(u.u_error) {
		st->st_ucnt = NULL; /*pcs*/
		st->st_flags = 0;	/*pcs*/
		return(-1);
	...
	i = btoc(st->st_npgs * sizeof(use_t));
	st->st_ucnt = (use_t *)sptalloc(i); /*pcs*/
...
register int	lowblk;	/* Low block number of area to delete.	*/
{
	...
	st = &swaptab[smi];
	if ((st->st_flags & ST_INDEL) == 0) { 					/*pcs*/
		if ((availsmem - st->st_npgs) < tune.t_minasmem) {	/*pcs*/
			printf("swapdel - too few free pages");			/*pcs*/
			u.u_error = ENOMEM;								/*pcs*/
			return -1;										/*pcs*/
		} else {											/*pcs*/
			availsmem -= st->st_npgs;						/*pcs*/
			/* Set the delete flag */
			st->st_flags |= ST_INDEL;						/*pcs*/
		}													/*pcs*/
	}
	/*	Clean up its pages.
...
register dbd_t	*dbd;
{
	...
	if ((kernvaddr = (caddr_t *)pgalloc(1, PG_V|PG_KW, -1)) == NULL) /*pcs*/
		return;

	if (ptmemall(rp, pt, 1, 0)) {
		pgfree(kernvaddr, 1, 0);	/*pcs*/
		return;
	}

	kernpte = (pte_t *)svtopte(kernvaddr);
/*	kernpte->pgm.pg_pfn = pt->pgm.pg_pfn;*/
	kernpte->pgi.pg_pte &= ~POFFMASK;		/*pcs*/
	kernpte->pgi.pg_pte |= pt->pgm.pg_pfn;	/*pcs*/
	clrcache();								/*pcs*/
...
register int size;
{
	...
	if ((st - swaptab) == comswapsmi && ownswap==0) { /*pcs*/
		i = diskless_swalloc(size);					/*pcs*/
		return i;									/*pcs*/
	}												/*pcs*/
