static char *_Version = "@(#) RELEASE:  1.1  Oct 21 1986 /usr/sys/io/tt1.c ";
...
register struct tty *tp;
{
    ...
    if ((u.u_fmode & FNDELAY)==0 && u.u_procp->p_nice <= NINTER) /*pcs*/
        u.u_procp->p_inter = 5;                                 /*pcs*/
    ...
    while (u.u_count) {
        if (tp->t_outq.c_cc > tthiwat[tp->t_cflag&CBAUD]) {
            ...
            while (tp->t_outq.c_cc > tthiwat[tp->t_cflag&CBAUD]) {
                (*tp->t_proc)(tp, T_OUTPUT); /*pcs*/
                if (tp->t_outq.c_cc) { /*pcs*/
                    tp->t_state |= OASLP;
...
register struct tty *tp;
{
    ...
    if (nc < cfreelist.c_size || (flg & (INLCR|IGNCR|ICRNL|IUCLC))) {
        ...
        tp->t_rbuf.c_ptr = scf->c_data; /*pcs*/
    ...
    if (!delct && tp->t_delct && (tp->t_cflag & TTYSEL)) {      /*pcs*/
...
} ucp;
{
    ...
    if (ncode == 0) {
        ...
        if (!(flg&OPOST)) {
            sysinfo.outch++;
            putc((short)ucp.ch, &tp->t_outq); /*pcs*/
            return;
        }
        cp = (unsigned char *)&lobyte(loword(ucp.ch)); /*pcs*/
        scf = NULL;
    } else {
        if (!(flg&OPOST)) {
            sysinfo.outch += ncode;
...
redo: /*pcs*/
    ...
    if (tp->t_outq.c_cc <= ttlowat[tp->t_cflag&CBAUD]) { /*pcs*/
        if (tp->t_state&OASLP) {            /*pcs*/
            tp->t_state &= ~OASLP;
...
{
    ushort  chg;
    register struct cblock *scf; /*pcs*/
    register s;
    ...
    case LDCLOSE:
        ...
        if (tp->t_tbuf.c_ptr) {
            ...
            tp->t_tbuf.c_count = 0; /*pcs*/
            tp->t_tbuf.c_size = 0; /*pcs*/
        }
        if (tp->t_rbuf.c_ptr) {
            putcf(CMATCH((struct cblock *)tp->t_rbuf.c_ptr));
            tp->t_rbuf.c_ptr = NULL;
            tp->t_rbuf.c_count = 0; /*pcs*/
            tp->t_rbuf.c_size = 0; /*pcs*/
        }
        break; /*pcs*/
